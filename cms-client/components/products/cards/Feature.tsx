...
import { IFeature, IProductFeature, IProductFeatureValue, ProductUpdateFeatureRequest } from '@/types/api';
import Input from '../../inputs/Input'

interface IProps {
    className: string;
    item: IProductFeature;
    index: number;
    onFeatureValuesUpdate: (id: number, data: Pick<ProductUpdateFeatureRequest, "createFeatureValues" | "deleteFeatureValues" | "updateFeatureValues" | "reorderFeatureValue">) => Promise<boolean>;
    onItemsInputChange: (e: React.ChangeEvent<HTMLInputElement>) => void;
    onFeatureUpdate: (item: IProductFeature) => void;
...
export default function Feature({ className, item, index, onItemsInputChange, onDragStart: onDragFeatureStart, onDragOver: onDragFeatureOver, onDragEnd: onDragFeatureEnd, setSelected: setFeatureSelected, onFeatureUpdate, onFeatureRemove, onFeatureValuesUpdate }: IProps) {
    ...
    const onDragEnd = async (e: React.DragEvent) => {
        ...
        if (selected.position !== currentPosition) {
            await onFeatureValuesUpdate(item.id, { reorderFeatureValue: { id: selected.id, position: currentPosition } })
        }
    ...
    const onDragOver = (e: React.DragEvent, itemValue: IProductFeatureValue, i: number) => {
        e.preventDefault();
    ...
    const onFeatureValueChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        const [field, index] = e.target.name.split("_")

        setState(prev => ({ ...prev, values: state.values.map(c => c.id.toString() === index ? { ...c, [field as "key" | "value"]: e.target.value } : c) }))
    }

    const onRemoveFeatureValue = (id: number) => {
        setState(prev => ({ ...prev, values: state.values.filter(c => c.id !== id) }))
    }

    const onAddFeatureValue = () => {
        if (state.newFeatureKey === "") return
        if (state.newFeatureValue === "") return
        if (state.values.find(c => c.key === state.newFeatureKey)) {
            return toast.error("Такая характеристика уже создана")
        }

        setState(prev => ({ ...prev, newFeatureKey: "", newFeatureValue: "", values: [...state.values, { id: Math.random(), key: prev.newFeatureKey, value: prev.newFeatureValue, position: state.values.length }] }))
    }

    const mustBeSaved = useMemo(() => state.values.length !== item.values.length || state.values.some(c => item.values.some(a => a.id === c.id && (a.key !== c.key || a.value !== c.value))), [state])
    ...
    const onSave = async () => {
        ...
        if (state.values.some(c => c.key === "")) {
            return toast.error("Название характеристики не может быть пустым")
        }
        ...
        const result = await onFeatureValuesUpdate(item.id, {
            ...
            deleteFeatureValues: deleteFeatureValues.length !== 0 ? deleteFeatureValues : undefined,
            updateFeatureValues: updateFeatureValues.length !== 0 ? updateFeatureValues : undefined,
        })
    ...
    return (
        <div
            ...
            onDragOver={(e) => onDragFeatureOver(e, item, index)}
        >
            <div className="flex-1">
                <div className="flex items-center gap-2 justify-between bg-white">
                    <div className="flex items-center justify-center w-10 h-10">
                        <div className="p-2 cursor-pointer" onMouseDown={() => setFeatureSelected(item)} onMouseUp={() => setFeatureSelected(null)}>
                    ...
                    <Input className="flex-1" type="text" placeholder="Заголовок характеристик" name={item.id.toString()} value={item.title} onChange={onItemsInputChange} onBlur={() => onFeatureUpdate(item)} />
                    <div className="flex items-center justify-center w-10 h-10">
                        <button className="p-2 rounded-md hover:bg-gray-50" onClick={onActionButtonClick}>
                            {state.edit ? (
                                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <div className="space-y-2">
                        {state.values.map((value, i) =>
                            ...
                            >
                                ...
                                <Input className="flex-1" type="text" placeholder="Характеристика" name={`key_${value.id.toString()}`} value={value.key} onChange={onFeatureValueChange} />
                                <Input className="flex-1" type="text" placeholder="Значение" name={`value_${value.id.toString()}`} value={value.value} onChange={onFeatureValueChange} />
                                <div className="flex items-center justify-center w-10 h-10">
                                    <button className={`p-2 rounded-md hover:bg-gray-50 ${state.values.length < 2 && "invisible"}`} onClick={() => onRemoveFeatureValue(value.id)}>
                                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            <path d="M10 11V17M14 11V17M4 7H20M19 7L18.133 19.142C18.0971 19.6466 17.8713 20.1188 17.5011 20.4636C17.1309 20.8083 16.6439 21 16.138 21H7.862C7.35614 21 6.86907 20.8083 6.49889 20.4636C6.1287 20.1188 5.90292 19.6466 5.867 19.142L5 7H19ZM15 7V4C15 3.73478 14.8946 3.48043 14.7071 3.29289C14.5196 3.10536 14.2652 3 14 3H10C9.73478 3 9.48043 3.10536 9.29289 3.29289C9.10536 3.48043 9 3.73478 9 4V7H15Z" stroke="black" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                                        </svg>
                                    </button>
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <Input className="flex-1" type="text" name="newFeatureKey" value={state.newFeatureKey} onChange={(e) => setState(prev => ({ ...prev, [e.target.name]: e.target.value }))} placeholder="Характеристика" />
                            <Input className="flex-1" type="text" name="newFeatureValue" value={state.newFeatureValue} onChange={(e) => setState(prev => ({ ...prev, [e.target.name]: e.target.value }))} placeholder="Значение" />
                            <div className="flex items-center justify-center w-10 h-10">
                                <button className="p-2 rounded-md hover:bg-gray-50" onClick={onAddFeatureValue}>
                        ...
                        <div className="">
                            <button className="w-full text-sm py-2 px-3 border-[1px] border-gray-300 hover:bg-gray-50 rounded-md font-medium" onClick={onSave}>Готово</button>
                        </div>
                    </div>
                </div>
            }
