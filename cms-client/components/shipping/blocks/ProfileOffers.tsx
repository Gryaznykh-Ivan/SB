...
export default function ProfileOffers({ onChange, deliveryProfileId, isDefault, connectOffers, disconnectOffers }: IProps) {
    ...
    return (
        <div className="rounded-md bg-white shadow-sm">
            <div className="flex justify-between items-center p-3 border-b-[1px]">
                <h2 className="font-semibold pl-3">Складское наличие</h2>
                <button className="text-gray-800 text-sm p-2 font-medium rounded-md hover:bg-gray-50 flex items-center" onClick={onPopupOpen}>
            ...
            <div className="flex space-x-4 p-3 border-b-[1px]">
                <SearchInput className="" placeholder="Поиск" onChange={onSearch} />
            </div>
            <div className="divide-y-[1px] max-h-96 overflow-y-auto">
                ...
                {data?.data &&
                    <>
                        {data.data.map(offer =>
                            <div key={offer.id} className="flex items-center px-3 py-2 space-x-4 hover:bg-gray-50 min-h-[50px]">
